...
def generate_templates(lang, question_id, qawiki_query, qawiki_endpoint, logger):
    ...
    get_alias_query = """SELECT ?alias WHERE {{ BIND(wd:{0} as ?item) OPTIONAL {{ ?item skos:altLabel ?alias. FILTER(LANG(?alias) = "en" || "es")}}}}"""
    ...
    if len(results_question_query) == 0:
        ...
        for ali in results_question_alias:
            if 'value' in ali[0]:
                alias_questions.append(ali[0]['value'].lower())
        ...
        return original_question, matched_mentions, question_template, query_template, visible_question, alias_questions
...
def get_templates(questions_ids, qawiki_endpoint_url, entity_prefix, boolean_values_dict, langs, logger):
    ...
    for question_id in questions_ids:
        ...
        try:
            ...
            for lang in langs:
                item[f"question_{lang}"], item[f"matches_{lang}"], item[f"question_template_{lang}"], item[f"query_template_{lang}"], item[f"visible_question_{lang}"], item["alias"] = generate_templates(lang, item["id"], raw_query, qawiki_endpoint_url, logger)
